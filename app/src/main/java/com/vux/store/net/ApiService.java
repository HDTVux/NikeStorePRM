...
import com.vux.store.model.ProductDetailResponse;
import com.vux.store.model.ReviewsResponse;
import com.vux.store.model.GetProductReviewResponse;
import com.vux.store.model.SubmitReviewRequest;
import com.vux.store.model.UserResponse;
import com.vux.store.model.WishlistResponse;
import com.vux.store.model.PromotionResponse; // NEW: Import PromotionResponse
import com.vux.store.model.SmartSuggestionResponse; // NEW: Import SmartSuggestionResponse

// NEW: Chat models
import com.vux.store.model.ChatResponse;
import com.vux.store.model.MessageListResponse;
import com.vux.store.model.UserChatListResponse;
...
public interface ApiService {
    ...
    Call<com.vux.store.model.ApiResponse> addToCart(
    ...
    Call<com.vux.store.model.CartResponse> getCart(@Query("user_id") int userId);

    // update item
    @FormUrlEncoded
    @POST("api.php?action=update_cart_item")
    Call<com.vux.store.model.ApiResponse> updateCartItem(
    ...
    Call<com.vux.store.model.ApiResponse> removeCartItem(@Field("item_id") int itemId);

    // clear cart
    @FormUrlEncoded
    @POST("api.php?action=clear_cart")
    Call<com.vux.store.model.ApiResponse> clearCart(@Field("user_id") int userId);

    // merge cart (POST JSON)
    @POST("api.php?action=merge_cart")
    Call<com.vux.store.model.ApiResponse> mergeCart(@Body Map<String, Object> body);

    // in ApiService.java
    @POST("api.php?action=create_order")
    Call<com.vux.store.model.ApiResponse> createOrder(@Body Map<String,Object> body);

    @POST("api.php?action=create_vnpay_payment")
    Call<com.vux.store.model.VnPayResponse> createVnPayPayment(@Body Map<String,Object> body);

    // get order status (server verification)
    @GET("api.php?action=get_order_status")
    Call<com.vux.store.model.OrderStatusResponse> getOrderStatus(@Query("order_id") int orderId);

    // trong ApiService.java
    @FormUrlEncoded
    @POST("api.php?action=verify_vnpay_return")
    Call<com.vux.store.model.ApiResponse> verifyVnpayReturn(@FieldMap Map<String, String> params);
